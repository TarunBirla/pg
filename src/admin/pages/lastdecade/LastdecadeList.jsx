...
export default function LastdecadeList() {
  ...
  const fetchnews = async () => {
    try {
      setLoading(true);
      const res = await http.get("/lastdecade");
      setNews(res.data);
      console.log(res.data);
    } catch (error) {
      console.error("Error fetching lastdecade:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeletenews = async (id) => {
    if (!window.confirm("Are you sure you want to delete this lastdecade?"))
      return;

    try {
      setDeleteLoadingId(id);
      await http.delete(`/lastdecade/${id}`);

      toast.success("lastdecade deleted successfully!");
      fetchnews();
    } catch (error) {
      console.error("Delete error:", error);
      toast.error(
        error.response.data.message || "Failed to delete lastdecade."
      );
    } finally {
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      ...
      <div className="max-w-7xl mx-auto bg-white rounded-lg shadow">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 p-4 md:p-6 border-b border-gray-200">
          ...
          <button
            onClick={() => navigate("/dashboard/lastdecade/add")}
                  ...
                  >
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() =>
                            navigate(`/dashboard/lastdecade/edit/${news.id}`)
                          }
